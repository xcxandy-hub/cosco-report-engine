...
| 通用编辑器 | `ReportDocument 1.5` 渲染、图表、图片、页面、模板展开、视觉精修、打印 | 报告领域公式、字段录入流程 |
| 页面模板层 | 5 套封面和 3 套页眉页脚的横竖毫米几何；展开为普通元素 | 持久化新元素类型、整页截图、机构页面复制 |
...
封面和页眉页脚模板同样是作者宏。`src/page-templates.ts` 用 `presetId` 和 `presetSlot` 标记展开结果，但输出仍只有原子元素。封面切换会保留现有标题、副标题和图片；页眉页脚切换只替换 role 或 `chrome-template:*` 元素。报告包显式包含 chrome 模板时，编译器不再生成默认 decorations，避免两套制度元素叠加。

## 4. 数据所有权

独立模式的事实所有权属于每个 text/chart/table 元素。编辑图 A 不会重算或修改图 B；修改表格不会改变相邻图表。组件旁“编辑数据”直接更新当前元素，完整文档作为本机事实源。

绑定模式继续作为旧报告包兼容能力使用单一事实源：

同一个事实只能在 `fields` 中录入一次。图表、KPI 和表格绑定同一字段或派生值。当前财务参考包已经切换为 `independent`，其文字、图表和表格直接保存各自内容，不使用 column bindings；只有保留兼容的旧 `bound` 报告包才走下面的字段、派生和规则链路。
...
模板几何也遵守方向隔离。每个封面和页眉页脚模板分别实现 portrait 与 landscape 坐标，不以等比缩放替代重新布局；模板单元测试逐项检查两种方向和页面边界。
