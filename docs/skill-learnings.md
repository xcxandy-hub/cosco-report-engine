...
| 学习收获 | 本项目落点 |
| --- | --- |
| PDF 版式不能只靠文本抽取判断，必须把最终页渲染为图片后目检。 | D3：`npm run qa:visual` 输出逐页 PNG 与 contact sheet；v1.8 再用 `qa:templates` 打印全部 16 种横竖模板组合。 |
...
模板的价值是降低初版构图成本，不是把自由编辑重新锁回母版。成熟机构报告的共同点是固定几何和反复出现的 running elements，但其专业感仍来自每个项目对图片、标题长度和内容密度的调整。

| 学习收获 | v1.8.0 落点 |
| --- | --- |
| 封面模板应固定构图关系，不固定具体内容。 | 5 套封面定义图片区、蒙版方向、Logo、标题和元数据槽位，切换时保留人工内容。 |
| 横版与竖版需要重新构图。 | 每套模板有两套毫米几何，测试证明坐标不同且不越界。 |
| 页眉页脚是导航系统，不是业务内容。 | 3 套 chrome 只替换 role 元素，正文、图表和表格保持不变。 |
| 模板必须可拆解。 | 应用结果只有 text/box/divider/image 等普通元素，可继续选中微调。 |
| 品牌资产要兼顾暗底和亮底。 | 内置透明彩色、透明白色和项目横向组合三个 PNG，并核验 alpha。 |
| 学习机构设计不等于复制机构资产。 | 年报只提炼方法；模板起始图使用 Public Domain/CC0 本地素材并保留来源说明。 |
